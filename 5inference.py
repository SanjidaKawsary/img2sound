...
def soundlize(model, sample_steps, image_embedding, real_audio, device=None):
    bs, T, dim = real_audio.shape
    ...
    for batch in tqdm(dataloader, desc="Inference"):
        ...
        with torch.no_grad():
            samples = soundlize(
                ...
                image_embedding=img, real_audio=aud, device=device,
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Img2Soundscape Inference (with audio reference)")

    # VAE
    parser.add_argument("--vae-model-path", type=str,
                        default=os.path.join(BASE_DIR, "models", "stable-audio-open"),
                        help="Path to pretrained VAE (or HF repo id)")
